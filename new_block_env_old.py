...
class NewBlockEnv(gym.Env):
    ...
    # NOTE: This function modifies the state if the agent is standing on a positive block
    def _get_reward(self):
        ...
        for ix, dx in enumerate(viewport_iteration_arr):
            for iy, dy in enumerate(viewport_iteration_arr):
                ...
                if manhattan_dist == 0 and obs[ix, iy] == BlockType.POSITIVE:
                    # We are sitting on a positive block, eat it
                    self.eat(self.x, self.y)
                    reward += on_block_multiplier
