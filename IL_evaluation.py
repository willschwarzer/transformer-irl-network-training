...
@ray.remote(num_gpus=1/(NUM_AGENTS_PER_GPU*4))
def evaluate_random(env):
    random_policy = stable_baselines3.PPO("MlpPolicy", env, verbose=1)
    mean_random_ret, _ = evaluate_policy(random_model, env, n_eval_episodes=10)
...
def evaluate_ground_truth(env):
    ...
    mean_ground_truth_ret, _ = evaluate_policy(random_model, env, n_eval_episodes=10)
    return mean_ground_truth_ret

@ray.remote(num_gpus=1/(NUM_AGENTS_PER_GPU*4))
def evaluate_sirl(traj_rep, state_encoder, weights):
...
@ray.remote(num_gpus=1/(NUM_AGENTS_PER_GPU*4))
def evaluate_adversarial(weights, alg):
...
def main(args):
    ...
    state_batches_int = torch.Tensor(state_batches_np).to(torch.int64).cuda()
    ...
    for rollout_batch, traj_batch, weights in zip(rollout_batches, state_batches, weight_batches):
        ground_truth_env = NewBlockEnv(weights)
        # Random
        random_rets = ray.get([evaluate_random.remote(ground_truth_env) for i in range(1)])
        ...
        # Our method
        traj_rep = trajectory_encoder(traj_batch)
        sirl_rets = ray.get([evaluate_sirl(traj_rep, state_encoder, weights) for i in range(10)])
        ave_sirl_ret = np.mean(sirl_rets)
        
        # GAIL
        gail_rets = ray.get([evaluate_adversarial(weights, 'gail') for i in range(10)])
        ave_gail_ret = np.mean(gail_rets)
        
        # AIRL
        airl_ret = ray.get([evaluate_adversarial(weights, 'airl') for i in range(10)])
